...
Trả lời chi tiết, có con số thực tế.` }
        ...
        const qaReport = [
          ...
          `${totalArea > project.budget * 0.1 ? "✅" : "⚠️"} Diện tích xây dựng ${totalArea.toFixed(2)}m² / Ngân sách ${project.budget} triệu`,
        ...
        const pdfApiData = {
          ...
          land_size: `${project.landWidth}m × ${project.landLength}m (${area.toFixed(2)} m²)`,
          ...
          qa_report: qaReport,
          total_area: `${totalArea.toFixed(2)} m²`,
        ...
        if (!downloadUrl) {
          ...
          const coverInfo = [
            ...
            `Kích thước: ${project.landWidth}m × ${project.landLength}m (${area.toFixed(2)} m²)`,
          ...
          const costRows = [
            ["1", "Chi phí xây dựng phần thô", `${totalArea.toFixed(2)} m²`, `${buildCost.toLocaleString("vi-VN")}`],
            ["2", "Chi phí hoàn thiện ngoại thất", `${totalArea.toFixed(2)} m²`, `${Math.round(totalArea * 1.5).toLocaleString("vi-VN")}`],
            ["3", "Chi phí thiết kế nội thất", `${totalArea.toFixed(2)} m²`, `${interiorCost.toLocaleString("vi-VN")}`],
            ["4", "Hệ thống điện - nước", "1 hệ thống", `${Math.round(totalArea * 0.8).toLocaleString("vi-VN")}`],
            ["5", "Cảnh quan sân vườn", `${area.toFixed(2)} m²`, `${Math.round(area * 0.5).toLocaleString("vi-VN")}`],
  ...
  app.get("/api/projects/:id/download-pdf", async (req, res) => {
    ...
    try {
      ...
      doc.font(fnR).fontSize(12).fill("#000").text(`${project.landWidth}m x ${project.landLength}m (${area.toFixed(2)} m2)`, 140, infoY + 75, { width: coverLeftW - 60 });
      ...
      const pd = [
        ...
        ["Kich thuoc dat", `${project.landWidth}m x ${project.landLength}m = ${area.toFixed(2)} m2`],
      ...
      doc.y = tTop + 28;
      const rows = [
        ["1", "Chi phi xay dung phan tho", `${totalArea.toFixed(2)} m2`, `${buildCost.toLocaleString("vi-VN")}`],
        ["2", "Hoan thien ngoai that", `${totalArea.toFixed(2)} m2`, `${Math.round(totalArea * 1.5).toLocaleString("vi-VN")}`],
        ["3", "Thiet ke noi that", `${totalArea.toFixed(2)} m2`, `${interiorCost.toLocaleString("vi-VN")}`],
        ["4", "He thong dien - nuoc", "1 he thong", `${Math.round(totalArea * 0.8).toLocaleString("vi-VN")}`],
        ["5", "Canh quan san vuon", `${area.toFixed(2)} m2`, `${Math.round(area * 0.5).toLocaleString("vi-VN")}`],
  ...
  app.post("/api/projects/:id/send-email", async (req, res) => {
    ...
    try {
      ...
      const stepStatuses = (project.stepStatuses as Record<string, string>) || {};
      const unapprovedSteps = [];
      for (let s = 1; s <= 7; s++) {
        if (stepStatuses[String(s)] !== "approved") unapprovedSteps.push(s);
      }
      if (unapprovedSteps.length > 0) {
        const stepNames = unapprovedSteps.map(s => `Bước ${s}: ${STEP_NAMES[s]}`).join(", ");
        return res.status(400).json({ success: false, message: `Không thể gửi email. Các bước chưa được duyệt: ${stepNames}.` });
      }
...
Nếu khách yêu cầu thay đổi cấu trúc, hãy lịch sự giải thích rằng thay đổi cấu trúc cần quay lại Bước 3 và không thể thực hiện tại bước này.`
      ...
      const systemPrompt = `Bạn là AI trợ lý thiết kế kiến trúc & nội thất của BMT DECOR (CÔNG TY TNHH TMDV BMT DECOR). Director: Võ Quốc Bảo. Địa chỉ: 7/92 Thành Thái, P.14, Q.10, TP.HCM. Website: thicongtramsac.vn.

DANH TÍNH QUAN TRỌNG:
- Bạn là AI tư vấn, KHÔNG phải nhân viên hay tư vấn viên con người.
- KHÔNG tự xưng là "tôi sẽ gửi file", "tôi sẽ liên hệ lại", hay hứa hẹn bất kỳ hành động thực tế nào ngoài phạm vi AI.
- KHÔNG hứa gửi file PDF, tài liệu hay email qua chat. Người dùng tự gửi qua nút Gửi Email/Zalo trong giao diện Bước 7.
- Nếu ai hỏi về gửi file: hướng dẫn họ dùng nút "Gửi hồ sơ qua Email" hoặc "Gửi qua Zalo" ở Bước 7 của giao diện.
...
KHÔNG BAO GIỜ hứa hẹn hoặc thực hiện gửi file qua chat. Khi khách hỏi về gửi email/Zalo hồ sơ, hãy hướng dẫn họ sử dụng nút "Gửi qua Email" hoặc "Gửi qua Zalo" tại Bước 7 trong giao diện.

QUAN TRỌNG - Duyệt bước (Approve step):
Khi khách hàng nói "Duyệt", "OK duyệt", "Duyệt bước này", "Chấp thuận", hoặc bất kỳ ý muốn duyệt/chấp nhận bước hiện tại, bạn PHẢI bao gồm tag đặc biệt trong phản hồi: [APPROVE_STEP:N] (N là số bước hiện tại).
Ví dụ: Nếu đang ở bước 3 và khách nói "Duyệt", bạn phản hồi kèm tag [APPROVE_STEP:3] ở cuối tin nhắn.

RÀNG BUỘC HÀNH VI NGHIÊM NGẶT:
- KHÔNG được hứa hẹn thời hạn cụ thể kiểu "3 tiếng tôi sẽ gửi", "tôi sẽ hoàn thiện và gửi email cho bạn".
- KHÔNG đóng vai nhân viên hoặc tư vấn viên hứa hẹn giao file/tài liệu.
- KHÔNG lặp lại bất kỳ câu mẹo/tip cố định nào nếu đã từng nói câu đó trong cuộc hội thoại này.
- Khi có lỗi CAD ở Bước 3: giải thích rõ nguyên nhân lỗi (vd: phòng vượt chiều rộng đất) và HỎI xác nhận "Bạn có muốn tôi tạo lại không?" TRƯỚC khi đề xuất tạo lại.
- Chỉ hỗ trợ kỹ thuật thiết kế kiến trúc và nội thất.
...
${searchContext ? "Nếu có kết quả tìm kiếm phía trên, hãy tham khảo và trích dẫn nguồn khi phù hợp." : ""}`;
      ...
      let approveResult: { success: boolean; step: number; message: string } | null = null;
      const approveMatch = reply.match(/\[APPROVE_STEP:(\d+)\]/);
      if (approveMatch && project) {
        const approveStep = parseInt(approveMatch[1]);
        if (!isNaN(approveStep) && approveStep >= 1 && approveStep <= 7) {
          try {
            const currentStatus = ((project.stepStatuses as Record<string, string>) || {})[String(approveStep)];
            const stepResultField: Record<number, keyof typeof project> = {
              1: "siteRequirements", 2: "analysisResult", 3: "cadResult",
              4: "model3dResult", 5: "interiorResult", 6: "renderResult", 7: "pdfEstimate",
            };
            const hasResult = !!(project[stepResultField[approveStep]]);
            if (currentStatus === "completed" || (currentStatus === "processing" && hasResult)) {
              const statuses = { ...(project.stepStatuses as Record<string, string> || {}), [approveStep]: "approved" };
              const nextStep = Math.min(approveStep + 1, 7);
              await storage.updateProject(project.id, {
                stepStatuses: statuses,
                currentStep: nextStep,
                status: approveStep === 7 ? "completed" : "active",
              });
              project = await storage.getProject(project.id);
              approveResult = { success: true, step: approveStep, message: `Bước ${approveStep} đã được duyệt.` };
            } else {
              approveResult = { success: false, step: approveStep, message: `Bước ${approveStep} chưa được xử lý xong, không thể duyệt.` };
            }
          } catch (approveErr) {
            console.error("Auto approve step error:", approveErr);
            approveResult = { success: false, step: approveStep, message: "Duyệt bước thất bại. Vui lòng thử lại." };
          }
        }
      }

      const cleanReply = reply.replace(/\[SEND_EMAIL:[^\]]+\]/g, "").replace(/\[APPROVE_STEP:\d+\]/g, "").trim();
