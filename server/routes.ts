...
${searchContext ? "Nếu có kết quả tìm kiếm phía trên, hãy tham khảo và trích dẫn nguồn khi phù hợp." : ""}`;
      ...
      if (emailMatch && project) {
        ...
        if (!emailRegex.test(recipientEmail)) {
          ...
          const chatStepStatuses = (project.stepStatuses as Record<string, string>) || {};
          const chatUnapprovedSteps = [];
          for (let s = 1; s <= 7; s++) {
            if (chatStepStatuses[String(s)] !== "approved") chatUnapprovedSteps.push(s);
          }
          if (chatUnapprovedSteps.length > 0) {
            const stepNames = chatUnapprovedSteps.map(s => `Bước ${s}: ${STEP_NAMES[s]}`).join(", ");
            emailResult = { success: false, message: `Không thể gửi email. Các bước chưa được duyệt: ${stepNames}.` };
          } else
          try {
